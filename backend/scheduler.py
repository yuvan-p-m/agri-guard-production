...
from sms_service import send_sms_detailed
    ...
    try:
        ...
        for farmer in farmers:
            try:
                ...
                res = send_sms_detailed(mobile, msg)
                if not res.get("success"):
                    logger.warning(f"Morning weather SMS to farmer ({res.get('phone', mobile)}) notice: {res.get('message')}")
            except Exception as item_e:
                logger.error(f"Morning weather SMS item exception: {item_e}")
    except Exception as e:
        logger.error(f"job_morning_weather_exception: {e}")
    ...
    try:
        ...
        for farmer in farmers:
            try:
                ...
                res = send_sms_detailed(mobile, msg)
                if not res.get("success"):
                    logger.warning(f"Soil sensor SMS to farmer ({res.get('phone', mobile)}) notice: {res.get('message')}")
            except Exception as item_e:
                logger.error(f"Soil sensor SMS item exception: {item_e}")
    except Exception as e:
        logger.error(f"job_soil_sensors_exception: {e}")
    ...
    try:
        ...
        for farmer in farmers:
            try:
                ...
                res = send_sms_detailed(mobile, msg)
                if not res.get("success"):
                    logger.warning(f"Midday weather SMS to farmer ({res.get('phone', mobile)}) notice: {res.get('message')}")
            except Exception as item_e:
                logger.error(f"Midday weather SMS item exception: {item_e}")
    except Exception as e:
        logger.error(f"job_midday_weather_exception: {e}")
    ...
    try:
        ...
        for farmer in farmers:
            try:
                ...
                res = send_sms_detailed(mobile, msg)
                if not res.get("success"):
                    logger.warning(f"Evening tip SMS to farmer ({res.get('phone', mobile)}) notice: {res.get('message')}")
            except Exception as item_e:
                logger.error(f"Evening tip SMS item exception: {item_e}")
    except Exception as e:
        logger.error(f"job_evening_tips_exception: {e}")
...
def start_scheduler():
    ...
    try:
        ...
        logger.error(f"start_scheduler_exception: {e}")
    return _scheduler
