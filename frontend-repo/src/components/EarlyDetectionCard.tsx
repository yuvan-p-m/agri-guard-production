...
export const EarlyDetectionCard: React.FC<EarlyDetectionCardProps> = ({
  ...
  const cropTitle = (diagnosis.cropName as any)?.[currentLang] || diagnosis.cropName?.en || t('earlyDetection.defaultCrop');
  const diseaseTitle = (diagnosis.diseaseName as any)?.[currentLang] || diagnosis.diseaseName?.en || t('earlyDetection.defaultCondition');
  const confidenceVal = typeof diagnosis.confidence === 'number' ? diagnosis.confidence : 90;
  const isHealthy = diagnosis.diseaseName?.en?.toLowerCase().includes('healthy') || diagnosis.id === 'healthy-crop-check';

  const alertText = (diagnosis.earlyWarningAlert as any)?.[currentLang] 
    || (currentLang === 'en' ? diagnosis.earlyWarningAlert?.en : null)
    || (isHealthy
        ? t('earlyDetection.healthyAlertTemplate', { crop: cropTitle, confidence: confidenceVal })
        : t('earlyDetection.earlyWarningAlertTemplate', { crop: cropTitle, disease: diseaseTitle, confidence: confidenceVal }));

  const rawSymptoms = (diagnosis.symptoms as any)?.[currentLang] || (currentLang === 'en' ? diagnosis.symptoms?.en : null);
  const symptomsList = Array.isArray(rawSymptoms) && rawSymptoms.length > 0
    ? rawSymptoms
    : [
        t('earlyDetection.symptomEvidence1', { disease: diseaseTitle }),
        t('earlyDetection.symptomEvidence2', { crop: cropTitle }),
        t('earlyDetection.symptomEvidence3')
      ];
  const spreadRate = typeof diagnosis.spreadRiskRate === 'number' ? diagnosis.spreadRiskRate : 0;

  const stageDisplay = diagnosis.stage?.toLowerCase().includes('early')
    ? t('earlyDetection.stageEarly')
    : diagnosis.stage?.toLowerCase().includes('moderate')
    ? t('earlyDetection.stageModerate')
    : diagnosis.stage?.toLowerCase().includes('advanced')
    ? t('earlyDetection.stageAdvanced')
    : (diagnosis.stage || t('earlyDetection.stageEarly'));

  const pathogenTypeDisplay = diagnosis.pathogenType?.toLowerCase().includes('bacteri')
    ? t('earlyDetection.pathogenBacterium')
    : diagnosis.pathogenType?.toLowerCase().includes('fung')
    ? t('earlyDetection.pathogenFungus')
    : diagnosis.pathogenType?.toLowerCase().includes('virus')
    ? t('earlyDetection.pathogenVirus')
    : (diagnosis.pathogenType || t('earlyDetection.pathogenFungus'));
    ...
    <div className="bg-white/95 backdrop-blur-md rounded-3xl p-5 sm:p-7 shadow-xl border border-agri-200/80 animate-slide-up">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-gradient-to-r from-citrus-500/15 via-agri-50 to-emerald-50 border border-citrus-300/80 mb-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-white text-agri-900 border border-agri-300 shadow-2xs">
                <Award className="w-3 h-3 text-agri-600" /> {confidenceVal}% {t('earlyDetection.confidence')}
              </span>
            </div>
        ...
        <button
          ...
          aria-label={t('earlyDetection.viewPrescription')}
          className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-agri-800 hover:bg-agri-900 text-white text-xs font-bold shadow-md shadow-agri-800/20 transition-all transform hover:scale-105 cursor-pointer"
        >
          <FileCheck className="w-3.5 h-3.5 text-citrus-300" />
          <span>{t('earlyDetection.viewPrescription')}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-5 border-b border-slate-100">
        <div className="md:col-span-2">
          ...
          <p className="text-xs text-agri-800 italic font-mono mt-1 font-semibold flex items-center gap-1.5">
            ...
            <span>{t('earlyDetection.pathogen')}: {diagnosis.scientificName || t('earlyDetection.defaultPathogen')}</span>
          </p>
        </div>
        ...
        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 flex flex-col justify-between space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-500 font-medium">{t('earlyDetection.pathogenType')}:</span>
            <span className="font-bold px-2 py-0.5 bg-white rounded-md text-slate-800 border border-slate-200">
              {pathogenTypeDisplay}
            </span>
          </div>
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-500 font-medium">{t('earlyDetection.incubation')}:</span>
        ...
        <div className="p-3.5 rounded-2xl bg-rose-50/70 border border-rose-200/70">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-bold text-rose-900 flex items-center gap-1">
              ...
              {t('earlyDetection.spreadRisk')}
            </span>
            <span className="text-sm font-black text-rose-700">
              +{spreadRate}% {t('earlyDetection.loss')}
            </span>
          </div>
        ...
        <div className="p-3.5 rounded-2xl bg-agri-50/80 border border-agri-200/80 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-agri-950">
              {t('earlyDetection.yieldProtected')}
            </p>
            <p className="text-[11px] text-agri-800 leading-snug">
              {t('earlyDetection.interventionDesc')}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-100">
        <p className="text-xs font-bold text-slate-800 mb-2">
          {t('earlyDetection.pathogenFactors')}:
        </p>
