  ...
  return (
    <div className="min-h-screen relative flex flex-col justify-between font-sans bg-earth-50 text-slate-900 overflow-x-hidden">
      ...
      <header className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 pb-2 flex items-center justify-between">
        ...
        {/* Server Endpoint Config & 25-Language Switcher */}
            ...
            <div className="w-full max-w-lg bg-white/95 backdrop-blur-xl rounded-3xl p-5 sm:p-6 shadow-2xl border border-white/60">
              ...
              {activeTab === 'register' && (
                <form onSubmit={handleRegisterSubmit} className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-bold text-slate-800 mb-1">
                        {t('auth.phone')} <span className="text-slate-400 font-normal">(Optional for SMS)</span>
                      </label>
                      <div className="relative">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="expert">🔬 {t('auth.roleExpert')}</option>
                        <option value="vendor">🏬 {t('auth.roleVendor')}</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 focus:border-agri-600 bg-slate-50/70 font-bold text-slate-800"
                    >
                      <option value="Citrus (Orange / Lemon)">{t('commodities.orange', 'Citrus (Orange / Lemon / Mosambi)')}</option>
                      <option value="Tomato / Vegetables">{t('commodities.tomato', 'Tomato / Vegetables')}</option>
                      <option value="Paddy / Rice">{t('commodities.rice', 'Paddy / Rice')}</option>
                      <option value="Chilli & Spices">Chilli & Spices</option>
                      <option value="Wheat & Grains">{t('commodities.wheat', 'Wheat & Grains')}</option>
                      <option value="Cotton / Cash Crops">{t('commodities.cotton', 'Cotton / Cash Crops')}</option>
                    </select>
                  </div>
                  ...
                  >
                    <span>{isLoading ? 'Creating Account...' : t('auth.registerBtn')}</span>
                    <ArrowRight className="w-4 h-4" />
                  </button>
                </form>
              )}
              ...
              <div className="relative my-4">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white/95 px-3 text-slate-400 font-bold uppercase tracking-wider text-[11px]">
                    {t('auth.orDivider')}
                  </span>
                </div>
              </div>

              {/* Guest Explore Quick Option (Eye-Catching Golden Harvest Theme) */}
              <button
                type="button"
                onClick={() => {
                  onLoginSuccess({
                ...
                className="w-full py-3.5 px-4 sm:px-5 rounded-2xl bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:from-amber-500 hover:via-yellow-500 hover:to-amber-600 border-2 border-amber-300 hover:border-amber-400 text-amber-950 font-black text-sm sm:text-base shadow-md shadow-amber-500/25 hover:shadow-lg hover:shadow-amber-500/35 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 flex items-center justify-between gap-3 transition-all cursor-pointer group active:scale-[0.99]"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-amber-950/10 border border-amber-950/15 flex items-center justify-center group-hover:bg-amber-950/15 group-hover:scale-105 transition-all shrink-0">
                    <Sparkles className="w-5 h-5 text-amber-950 fill-amber-600/30" />
                  </div>
                  <div className="text-left">
                    <span className="block font-black text-amber-950 text-sm sm:text-base leading-tight">
                      {t('auth.guestFarmerBtn')}
                    </span>
                    <span className="block text-[11px] font-bold text-amber-900/90 leading-tight mt-0.5">
                      No account needed • Instant access
                    </span>
                  </div>
                </div>
                <div className="w-8 h-8 rounded-xl bg-amber-950/10 flex items-center justify-center group-hover:bg-amber-950/20 transition-all shrink-0">
                  <ArrowRight className="w-4 h-4 text-amber-950 transition-transform group-hover:translate-x-0.5" />
                </div>
              </button>
      ...
      <footer className="relative z-10 w-full max-w-7xl mx-auto px-4 py-3 text-center text-xs text-white/80 drop-shadow-sm font-medium">
        <p>AgriGuard AI • Next-Gen Agricultural Intelligence</p>
      </footer>
