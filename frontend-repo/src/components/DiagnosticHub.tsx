...
import type { DiseaseDiagnosis } from '../types';
...
export const DiagnosticHub: React.FC<DiagnosticHubProps> = ({
  ...
  // Quick symptom chips with localized label and insertion text
  const symptomTags = [
    { key: 'yellowing', label: t('diagnosticHub.symptomYellowing'), text: t('diagnosticHub.symptomTextYellowing') },
    { key: 'spots', label: t('diagnosticHub.symptomSpots'), text: t('diagnosticHub.symptomTextSpots') },
    { key: 'wilting', label: t('diagnosticHub.symptomWilting'), text: t('diagnosticHub.symptomTextWilting') },
    { key: 'powder', label: t('diagnosticHub.symptomPowder'), text: t('diagnosticHub.symptomTextPowder') },
    { key: 'curl', label: t('diagnosticHub.symptomCurl'), text: t('diagnosticHub.symptomTextCurl') },
    { key: 'rot', label: t('diagnosticHub.symptomRot'), text: t('diagnosticHub.symptomTextRot') },
  ];
  ...
  const handleSampleCropSelect = (disease: DiseaseDiagnosis) => {
    ...
    const langSymptoms = (disease.symptoms as any)?.[currLang] || disease.symptoms.en;
    if (langSymptoms?.length) {
      setSymptomText(langSymptoms.join(', '));
    }
  };
  ...
  // Realistic Voice Speech-To-Text Handler with 25-language locale awareness
    ...
    if (SpeechRecognition) {
      try {
        ...
        const speechLocales: Record<string, string> = {
          en: 'en-US', ta: 'ta-IN', te: 'te-IN', ml: 'ml-IN', kn: 'kn-IN',
          hi: 'hi-IN', bn: 'bn-IN', mr: 'mr-IN', gu: 'gu-IN', pa: 'pa-Guru-IN',
          ur: 'ur-PK', or: 'or-IN', as: 'as-IN', ne: 'ne-NP', si: 'si-LK',
          ar: 'ar-SA', fr: 'fr-FR', es: 'es-ES', pt: 'pt-BR', de: 'de-DE',
          it: 'it-IT', ru: 'ru-RU', ja: 'ja-JP', ko: 'ko-KR', zh: 'zh-CN'
        };
        const currentLangCode = i18n.language || 'en';
        recognition.lang = speechLocales[currentLangCode] || currentLangCode || 'en-US';
      ...
      setIsRecording(true);
      setTimeout(() => {
        const mockVoiceText = t('diagnosticHub.mockVoiceText');
    ...
    <div className="bg-white/95 backdrop-blur-md rounded-3xl p-5 sm:p-7 shadow-xl border border-agri-200/80 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2 bg-agri-50 px-3.5 py-2 rounded-2xl border border-agri-200 self-start sm:self-auto shadow-2xs">
          ...
          <span className="text-xs font-bold text-slate-700">{t('diagnosticHub.fieldAcreage')}</span>
          <div className="flex items-center gap-1">
            ...
            />
            <span className="text-xs font-bold text-agri-800">{t('common.acres')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-5">
        <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100/90 rounded-2xl border border-slate-200/80 mb-5">
          ...
          >
            <span>{t('diagnosticHub.uploadTab')}</span>
          </button>
          <button
            type="button"
          ...
          >
            <span>{t('diagnosticHub.voiceTab')}</span>
          </button>
        </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 text-left">
                  <div className="relative w-32 h-32 rounded-2xl overflow-hidden shadow-md border-2 border-agri-500 shrink-0 bg-slate-100">
                    <img
                      ...
                      alt={t('diagnosticHub.cropPreviewAlt')}
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-black">
                      <Check className="w-3.5 h-3.5" /> {t('diagnosticHub.ready')}
                    </div>
                    <p className="text-xs sm:text-sm font-extrabold text-slate-900">
                      {t('diagnosticHub.selectCrop')}: {(() => {
                        const d = cropDiseases.find(item => item.id === selectedCropId);
                        if (!d) return t('diagnosticHub.customPhoto');
                        const lang = i18n.language || 'en';
                        return (d.cropName as any)?.[lang] || d.cropName.en;
                      })()}
                    </p>
                    <div className="flex flex-wrap gap-2 pt-1">
                      <button
                        ...
                        aria-label={t('diagnosticHub.chooseFile')}
                        className="px-3 py-1.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-300 text-xs font-bold text-slate-700 flex items-center gap-1.5 shadow-2xs cursor-pointer"
                      >
                        <FolderOpen className="w-3.5 h-3.5" />
                        <span>{t('diagnosticHub.chooseFile')}</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => cameraInputRef.current?.click()}
                        aria-label={t('diagnosticHub.cameraScan')}
                        className="px-3 py-1.5 rounded-xl bg-agri-100 hover:bg-agri-200 border border-agri-300 text-xs font-bold text-agri-900 flex items-center gap-1.5 shadow-2xs cursor-pointer"
                      >
                        <Camera className="w-3.5 h-3.5 text-agri-700" />
                        <span>{t('diagnosticHub.cameraScan')}</span>
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="py-6 space-y-4">
                  ...
                  <div>
                    <h3 className="text-sm sm:text-base font-black text-slate-800">
                      {t('diagnosticHub.dropzoneTitle')}
                    </h3>
                    <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto leading-relaxed">
                      {t('diagnosticHub.dropzoneSubtitle')}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                    <button
                      ...
                      onClick={() => fileInputRef.current?.click()}
                      aria-label={t('diagnosticHub.chooseFile')}
                      className="px-4 py-2.5 rounded-2xl bg-white hover:bg-slate-50 border-2 border-agri-600 text-agri-900 text-xs sm:text-sm font-extrabold shadow-sm transition-all flex items-center gap-2 hover:scale-105 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      onClick={() => cameraInputRef.current?.click()}
                      aria-label={t('diagnosticHub.cameraScan')}
                      className="px-4 py-2.5 rounded-2xl bg-agri-700 hover:bg-agri-800 text-white text-xs sm:text-sm font-extrabold shadow-md shadow-agri-700/25 transition-all flex items-center gap-2 hover:scale-105 cursor-pointer"
                    >
          ...
          <div className="space-y-4">
            <div>
                ...
                >
                  {isRecording ? (
                    <>
                      ...
                      <span>{t('diagnosticHub.stopRecording')}</span>
                    </>
                  ) : (
                    <>
                      <Mic className="w-3.5 h-3.5 text-agri-700" />
                      <span>{t('diagnosticHub.startRecording')}</span>
                    </>
                  )}
                </button>
              </div>
              ...
              {isRecording && (
                <div className="mb-2 p-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-semibold flex items-center justify-between animate-fade-in">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{t('diagnosticHub.recordingActive')}</span>
                  </div>
                  <div className="flex items-center gap-1">
              ...
              <textarea
                ...
                placeholder={t('diagnosticHub.symptomsPlaceholder')}
            ...
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 mb-2">
                {t('diagnosticHub.describeSymptoms')}
              </p>
          ...
          >
            {isAnalyzing ? (
              <>
                ...
                <span>{t('diagnosticHub.analyzingBtn')}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-5 h-5 text-citrus-300" />
                <span>{t('diagnosticHub.analyzeBtn')}</span>
              </>
            )}
          </button>
        </div>
