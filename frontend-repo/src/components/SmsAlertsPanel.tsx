...
import type { SmsAlert } from '../types';
...
export const SmsAlertsPanel: React.FC<SmsAlertsPanelProps> = ({
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubStatus(`✅ ${t('smsModal.success', 'Subscribed!')} ${res?.message || ''}`);
      setSubPhone('');
    } catch (err: any) {
      setSubStatus(`⚠️ ${t('smsModal.invalid', 'Failed to subscribe. Please try again.')}`);
    } finally {
    ...
    <div id="sms-panel" className="bg-white/95 backdrop-blur-md rounded-3xl p-5 sm:p-6 shadow-xl border border-agri-200/80">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm sm:text-base font-extrabold text-slate-900">
              {t('smsPanel.title', 'Live SMS & Advisory Feed')}
            </h3>
            <p className="text-[11px] text-slate-500 font-medium">
              {t('smsPanel.subtitle', 'Real-time micro-climate warnings, weather forecasts, and AI pest triggers delivered directly to farmer SIMs')}
            </p>
          </div>
        </div>
        ...
        <span className="text-[11px] font-bold text-agri-800 bg-agri-50 px-2.5 py-1 rounded-full border border-agri-200 self-start sm:self-auto">
          {t('smsPanel.liveNotificationBadge', 'Active SMS Gateway')}
        </span>
      </div>
      ...
      <form onSubmit={handleSubscribe} className="mt-4 p-3 rounded-2xl bg-agri-50/70 border border-agri-200/60 space-y-2">
        <div className="flex items-center gap-1.5 text-xs font-bold text-agri-950">
          ...
          <span>{t('smsModal.title', 'Subscribe to Daily Micro-Climate & Risk SMS Alerts')}</span>
        </div>
        <div className="flex items-center gap-2">
          <input
            type="tel"
            placeholder={t('smsModal.placeholder', 'Enter mobile number (+91...)')}
            value={subPhone}
          ...
          >
            {isSubmitting ? t('smsModal.saving', 'Saving...') : t('smsDemo.subscribeButton', 'Subscribe')}
          </button>
        </div>
        {subStatus && (
      ...
      <div className="mt-4 space-y-3">
        {alerts.map((alert) => {
          const alertTitle = typeof alert.title === 'string' ? alert.title : (alert.title as any)?.[currentLang] || alert.title?.en || '';
          const alertMessage = typeof alert.message === 'string' ? alert.message : (alert.message as any)?.[currentLang] || alert.message?.en || '';
          const alertAction = alert.actionRequired ? (typeof alert.actionRequired === 'string' ? alert.actionRequired : (alert.actionRequired as any)?.[currentLang] || alert.actionRequired?.en || '') : '';
            ...
            >
              ...
              {alertAction && (
                <div className="mt-2 text-[11px] text-agri-950 font-semibold bg-white/80 p-2 rounded-xl border border-slate-200/60">
                  👉 <strong>{t('treatment.immediateSteps', 'Action')}:</strong> {alertAction}
                </div>
              )}
              ...
              <div className="mt-3 pt-2 border-t border-slate-200/60 flex items-center justify-between">
                ...
                >
                  ...
                  <span>{t('smsDemo.previewPhone', 'Preview on Phone')}</span>
                </button>
                ...
                >
                  ...
                  <span>{t('smsDemo.sendTestSms', 'Send Test SMS')}</span>
                </button>
              </div>
