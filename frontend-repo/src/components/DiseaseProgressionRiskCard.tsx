...
import type { ProgressionRisk } from '../types';
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
        ...
        >
          <span>
            {riskLevel === 'No Risk'
              ? t('progressionRisk.noRisk')
              : riskLevel === 'Low Risk'
              ? t('progressionRisk.lowRisk')
              : riskLevel === 'Medium Risk'
              ? t('progressionRisk.moderateRisk')
              : riskLevel === 'Severe Outbreak Risk'
              ? t('progressionRisk.severeRisk')
              : t('progressionRisk.highRisk')}
          </span>
        </span>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {progressionRisk.progression_stage && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-100 text-slate-800 text-xs font-bold border border-slate-200 shadow-2xs">
              ...
              <span>{t('progressionRisk.progressionStage')}: <strong>{progressionRisk.progression_stage}</strong></span>
            </span>
          )}
          {progressionRisk.vulnerability_window && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-50 text-amber-900 text-xs font-bold border border-amber-200 shadow-2xs">
              ...
              <span>{t('progressionRisk.vulnerabilityWindow')}: <strong>{progressionRisk.vulnerability_window}</strong></span>
            </span>
          )}
        </div>
      )}
      ...
      {!isHealthy && Array.isArray(progressionRisk.pathology_factors) && progressionRisk.pathology_factors.length > 0 && (
        <div className="p-4 bg-slate-50/90 rounded-2xl border border-slate-200/80 space-y-2">
          <span className="text-[11px] font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
            ...
            {t('progressionRisk.riskReasoning')}
          </span>
          ...
          <div className="p-3 bg-blue-50/70 rounded-2xl border border-blue-200/70 space-y-1.5 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-blue-900 uppercase tracking-wider">
                📡 {t('progressionRisk.iotTelemetry')}
              </span>
            ...
            <div className="grid grid-cols-3 gap-1 text-[11px] font-medium text-slate-700">
              <div><span className="text-slate-500">{t('progressionRisk.humidity')}:</span> <strong>{sensorSnapshot?.humidity ?? '--'}%</strong></div>
              <div><span className="text-slate-500">{t('progressionRisk.temperature')}:</span> <strong>{sensorSnapshot?.temperature ?? '--'}°C</strong></div>
              <div><span className="text-slate-500">{t('progressionRisk.moisture')}:</span> <strong>{sensorSnapshot?.moisture ?? '--'}%</strong></div>
            </div>
          </div>
          ...
          <div className="p-3 bg-teal-50/70 rounded-2xl border border-teal-200/70 space-y-1.5 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-teal-900 uppercase tracking-wider flex items-center gap-1">
                ...
                {t('progressionRisk.weatherTelemetry')}
              </span>
            ...
            <div className="grid grid-cols-3 gap-1 text-[11px] font-medium text-slate-700">
              <div><span className="text-slate-500">{t('progressionRisk.airTemp')}:</span> <strong>{weatherTemp !== null ? `${weatherTemp}°C` : '--'}</strong></div>
              <div><span className="text-slate-500">{t('progressionRisk.rainChance')}:</span> <strong>{weatherRainProb !== null ? `${weatherRainProb}%` : '--'}</strong></div>
              <div className="truncate"><span className="text-slate-500">{t('progressionRisk.wind')}:</span> <strong>{weatherWind !== null ? `${weatherWind}k` : '--'}</strong></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="pt-2 border-t border-slate-100 flex items-center gap-1.5 text-[11px] text-slate-500 font-medium leading-tight">
        ...
        <span>
          {t('progressionRisk.epidemiologicalNote')}
        </span>
      </div>
    </div>
  );
