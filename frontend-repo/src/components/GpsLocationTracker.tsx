...
export const GpsLocationTracker: React.FC<GpsLocationTrackerProps> = ({
  ...
  const displayLabel = label || t('fieldData.locationLabel', 'Farm / Field Location');
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-black tracking-tight text-slate-800">
                {gpsStatus === 'granted' 
                  ? t('gps.liveActive', 'Live GPS Tracking Active') 
                  : gpsStatus === 'requesting' 
                  ? t('gps.detecting', 'Requesting GPS Permission...') 
                  : gpsStatus === 'denied'
                  ? t('gps.denied', 'GPS Permission Denied')
                  : t('gps.title', 'GPS Device Location')}
              </span>
            </div>
          </div>
          ...
          >
            {gpsStatus === 'requesting' ? (
              <>
                ...
                <span>{t('gps.locating', 'Locating...')}</span>
              </>
            ) : (
              <>
                <RefreshCw className="w-3 h-3 text-agri-700" />
                <span>{gpsStatus === 'granted' ? t('gps.reTrack', 'Re-Track GPS') : t('gps.allowGps', 'Allow GPS')}</span>
              </>
            )}
          </button>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="block text-xs font-bold text-slate-800">
            {displayLabel} <span className="text-slate-400 font-normal">{t('gps.autoTrackedDesc', '(Auto-tracked or type manually)')}</span>
          </label>
          {isManualOverride && (
            <span className="text-[10px] text-agri-700 font-bold bg-agri-50 px-1.5 py-0.5 rounded border border-agri-200">
              {t('gps.customLocation', 'Custom Location')}
            </span>
          )}
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('fieldData.locationSearchPlaceholder', 'e.g. Nagpur / Coimbatore / Punjab')}
        ...
        <p className="mt-1 text-[10px] text-slate-500">
          💡 {t('gps.savedCoordsNotice', 'GPS coordinates are saved to deliver localized crop disease warnings and weather forecasting.')}
        </p>
      </div>
