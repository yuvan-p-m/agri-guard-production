...
import type { UserProfile, MandiRecord, PriceForecastResponse, CropAlertResponse } from '../types';
...
export const MarketplaceTab: React.FC<MarketplaceTabProps> = ({ user }) => {
  ...
  const fetch_all_data = async (crop_val: string, state_val: string) => {
    ...
    try {
      const alert_res = await marketplaceAPI.getCropAlert(crop_val, i18n.language);
    ...
    try {
      const mandi_res = await marketplaceAPI.getMandiPrices(crop_val, state_val, i18n.language);
      ...
      set_error_message(t('marketplace.noPriceData', 'No price data available for your crop in your state today. Try again tomorrow.'));
    } finally {
      set_is_loading_mandi(false);
    }

    try {
      const forecast_res = await marketplaceAPI.getPriceForecast(crop_val, state_val, i18n.language);
  ...
  }, [selected_crop, selected_state, i18n.language]);
        ...
        <div className="mt-6 pt-5 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              ...
              <span>{t('marketplace.selectState')}:</span>
            </label>
            <select
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              ...
              <span>{t('marketplace.selectCommodity')}:</span>
            </label>
            <select
              value={selected_crop}
          ...
          <div className="space-y-1 flex-1">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h4 className="text-sm sm:text-base font-black text-amber-950">
                {t('marketplace.cropCalendarAdvisory', 'Crop Calendar Advisory')} — {crop_alert.crop}
              </h4>
              <span className="px-2.5 py-0.5 rounded-full bg-amber-200/80 text-amber-900 text-[10px] font-black uppercase tracking-wider">
                {t(`marketplace.alertTypes.${crop_alert.alert_type}`, crop_alert.alert_type.replace(/_/g, ' '))}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">
                {t('marketplace.liveMandiPriceComparison', 'Live Mandi Price Comparison')}
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                {t('marketplace.realtimeArrivals', 'Real-time APMC market arrivals for')} {t(`commodities.${selected_crop}`, selected_crop).toUpperCase()} {t('marketplace.in', 'in')} {t(`states.${selected_state.toLowerCase().replace(/\s+/g, '_')}`, selected_state)}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-emerald-100 block">
                      {t('marketplace.bestMarketToday', 'Best Market Today')}
                    </span>
                    <p className="text-base sm:text-lg font-black tracking-tight">
                      {t('marketplace.bestMarketToday', 'Best Market Today')}: <span className="text-citrus-300">{best_mandi.market}</span> ({best_mandi.district}) {t('marketplace.at', 'at')} ₹{best_mandi.modal_price}/quintal
                    </p>
                  </div>
                </div>
            ...
            {is_loading_mandi && (
              <div className="p-10 text-center space-y-3">
                ...
                <p className="text-sm font-black text-slate-800">{t('marketplace.fetchingRates', 'Fetching live mandi rates from data.gov.in...')}</p>
              </div>
            )}
                    ...
                    <tr className="bg-slate-100/80 text-slate-600 uppercase text-[11px] font-black tracking-wider border-b border-slate-200">
                      ...
                      >
                        <div className="flex items-center gap-1">
                          <span>{t('marketplace.table.market', 'Market')}</span>
                      ...
                      >
                        <div className="flex items-center gap-1">
                          <span>{t('marketplace.table.district', 'District')}</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400" />
                        </div>
                      </th>
                      <th
                      ...
                      >
                        <div className="flex items-center justify-center gap-1">
                          <span>{t('marketplace.table.minPrice', 'Min Price')}</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400" />
                        </div>
                      </th>
                      <th
                      ...
                      >
                        <div className="flex items-center justify-center gap-1">
                          <span>{t('marketplace.table.maxPrice', 'Max Price')}</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400" />
                        </div>
                      </th>
                      <th
                      ...
                      >
                        <div className="flex items-center justify-end gap-1">
                          <span>{t('marketplace.table.modalPrice', 'Modal Price (₹/Q)')}</span>
                          <ArrowUpDown className="w-3 h-3 text-slate-400" />
                        </div>
                      </th>
                      <th className="py-3 px-4 text-right">{t('marketplace.table.date', 'Date')}</th>
                    </tr>
                  </thead>
      ...
      <div className="bg-white/95 backdrop-blur-md rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200 space-y-5">
        <div className="flex items-start justify-between pb-4 border-b border-slate-100 gap-3">
          <div className="flex items-start gap-3">
            <div className="p-2.5 rounded-2xl bg-agri-700 text-white shadow-md shrink-0 mt-0.5">
              <BarChart2 className="w-5 h-5" />
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">
                  {t('marketplace.priceForecast', 'Demand Forecasting & 3-Week Price Trend')}
                </h3>
                <span className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/50 border border-amber-300/90 dark:border-amber-700 text-amber-950 dark:text-amber-200 font-extrabold text-[10px] sm:text-[11px] shadow-2xs">
                  <Sparkles className="w-3 h-3 text-amber-600 dark:text-amber-400 shrink-0" />
                  <span>{t('marketplace.mlAttributionMadhawa', 'Price prediction by MadhawaDG/farmer-price-predictor ML MODEL')}</span>
                </span>
              </div>
        ...
        {is_forecast_expanded && (
          <div className="space-y-6">
            {is_loading_forecast && (
              <div className="p-10 text-center space-y-3">
                <RefreshCw className="w-8 h-8 text-agri-700 animate-spin mx-auto" />
                <p className="text-sm font-black text-slate-800">{t('marketplace.calculatingForecast', 'Calculating rolling averages & forecast...')}</p>
              </div>
            )}
              ...
              <>
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                      <TrendingUp className="w-3.5 h-3.5 text-agri-700" />
                      <span>{t('marketplace.threeWeekProjections', '3-Week Forward Rate Projections (₹/Quintal)')}</span>
                    </span>
                    <span className="text-[11px] font-bold text-slate-500">
                      {t('marketplace.basedOnRollingAvg', 'Based on 8-week weighted rolling averages')}
                    </span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {forecast_data.forecast.map((f_price, idx) => {
                      const week_label = `${t('marketplace.week', 'Week')} +${idx + 1}`;
                      const days_label = `+${(idx + 1) * 7} ${t('marketplace.days', 'Days')}`;
                      ...
                      return (
                        <div
                          key={idx}
                          className="p-4 rounded-2xl bg-gradient-to-br from-agri-50/70 via-white to-slate-50 border border-agri-200/80 shadow-sm flex flex-col justify-between"
                        >
                          <div className="flex items-center justify-between pb-2 border-b border-agri-100">
                            <div>
                              <span className="text-[10px] font-black uppercase tracking-wider text-agri-900 block">
                                {week_label}
                              </span>
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
                              ...
                              {diff > 0 ? `+${diff_pct}%` : diff < 0 ? `${diff_pct}%` : t('marketplace.stable', 'Stable')}
                            </span>
                          </div>

                          <div className="mt-3">
                            <div className="text-2xl font-black font-mono text-agri-950">
                              ₹{f_price.toLocaleString('en-IN')}
                              <span className="text-xs font-semibold text-slate-500 ml-1">/ Q</span>
                            </div>
                            <p className="text-[11px] text-slate-500 font-medium mt-1">
                              {diff > 0 ? `▲ ₹${Math.abs(diff)} ${t('marketplace.increaseExpected', 'increase expected')}` : diff < 0 ? `▼ ₹${Math.abs(diff)} ${t('marketplace.decreaseExpected', 'decrease expected')}` : `→ ${t('marketplace.steadyPriceLevels', 'Steady price levels')}`}
                            </p>
                          </div>
                        </div>
                      );
                ...
                {(() => {
                  ...
                  if (trend_type === 'RISING') {
                    ...
                    card_bg = 'bg-gradient-to-r from-teal-50 via-slate-50 to-teal-50 border-teal-300 text-teal-950';
                    badge_color = 'bg-teal-700 text-white';
                    trend_icon = <Minus className="w-5 h-5 text-teal-700" />;
                  }
                    ...
                    <div className={`p-6 rounded-3xl border-2 ${card_bg} shadow-md space-y-4`}>
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 block">
                              {t('marketplace.outlookVerdict', '3-Week Outlook Verdict')}
                            </span>
                        ...
                        <div className="text-right">
                          <span className={`px-3.5 py-1 rounded-full text-xs font-black shadow-xs ${badge_color}`}>
                            {pct > 0 ? `+${pct}%` : `${pct}%`} {t('marketplace.expected', 'Expected')}
                          </span>
                          <p className="text-xs font-mono font-black mt-1">
                            {t('marketplace.expectedPriceInThreeWeeks', 'Expected price in 3 weeks')}: ₹{final_forecast_price}/Q
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="pt-3 border-t border-slate-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
                        <p className="font-black text-slate-800">
                          🎯 {t('marketplace.recommendation', 'Recommendation')}: <span className="font-extrabold">{forecast_data.recommendation}</span>
                        </p>
                        <p className="text-[11px] text-slate-500 font-medium">
                          {t('marketplace.basedOnRecords', 'Based on price records from last 8 weeks across')} {t(`states.${selected_state.toLowerCase().replace(/\s+/g, '_')}`, selected_state)}
                        </p>
                      </div>
                    </div>
                  );
