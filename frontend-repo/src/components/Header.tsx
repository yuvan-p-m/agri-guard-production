...
import type { UserProfile, WeatherInfo } from '../types';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-[100] w-full bg-white/85 backdrop-blur-md border-b border-agri-200/60 shadow-xs transition-all">
        ...
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-4">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="hidden md:inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-agri-100 text-agri-800 border border-agri-300">
                  <ShieldCheck className="w-3 h-3 text-agri-600" /> {t('header.aiVersion')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3 px-3.5 py-1.5 rounded-2xl bg-agri-50/90 border border-agri-200/80 text-xs shadow-inner">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              ...
              <span className="text-slate-500 font-semibold">{t('header.currentLocation')}:</span>
              <span className="text-slate-900 font-extrabold">
                {weather.city.toLowerCase().includes('fetching') ? t('header.fetchingLocation') : weather.city}
              </span>
              {!weather.city.toLowerCase().includes('fetching') && (
                <span className="text-agri-800 font-extrabold ms-1">{weather.tempC}°C</span>
              )}
            </div>
            <div className="h-3 w-px bg-agri-300" />
            <div className="text-slate-600 font-medium">
              {t('header.humidity')}: <strong className="text-slate-800">{weather.humidity}%</strong>
            </div>
            <div className="h-3 w-px bg-agri-300" />
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            ...
            <button
              ...
              title={t('header.alertsTitle')}
              aria-label={t('header.alertsTitle')}
            >
              <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-slate-800" />
              {unreadNotifications > 0 && (
                <span className="absolute -top-1 ltr:-right-1 rtl:-left-1 w-4 h-4 bg-rose-500 text-white rounded-full text-[10px] font-black flex items-center justify-center">
                  {unreadNotifications}
                </span>
              )}
            </button>
