...
interface FertilizerRecommendation {
  no_fertilizer_needed: boolean;
  no_fertilizer_message?: string;
  items: FertilizerItem[];
  ph_amendment?: PhAmendment | null;
  standards?: {
    icar_standard?: string;
    live_soil_telemetry?: string;
  };
}
...
export const CropRecommendationTab: React.FC<CropRecommendationTabProps> = ({
  ...
  // Re-fetch recommendations on load and whenever the selected language changes
  useEffect(() => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-agri-100 text-agri-800 text-xs font-black mb-2 border border-agri-200">
              ...
              <span>{`Random Forest ML Engine (${data?.total_crops || 42} Crops / ${data?.accuracy || '99.40% Accuracy'})`}</span>
            </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-100 flex flex-col sm:flex-row gap-2.5">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Enter City, District, or GPS (e.g. Nagpur, Maharashtra)"
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isLoading ? t('cropRecommendation.analyzing') : t('cropRecommendation.refreshBtn')}</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-gradient-to-r from-emerald-50 via-agri-50 to-teal-50 border border-emerald-200/90 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-sm">
            ...
            <span className="text-[11px] font-bold px-3 py-1 bg-white text-emerald-900 rounded-full border border-emerald-300 shadow-2xs">
              ⚡ {t('cropRecommendation.liveSensors')}
            </span>
          </div>
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-black uppercase tracking-wider text-slate-700">
                  {t('cropRecommendation.featuresFed')}
                </h3>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                  <CloudSun className="w-3 h-3 text-blue-500" /> Weather
                </span>
                ...
                <p className="text-[10px] text-slate-500">
                  {data.input_features?.humidity ?? data.weather.humidity}% Humidity
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                  <Droplets className="w-3 h-3 text-cyan-500" /> {t('cropRecommendation.soilPhMoisture')}
                </span>
                <p className="font-black text-slate-800 text-sm">
                  pH {data.input_features?.ph ?? data.sensor_snapshot.ph ?? '--'}
                </p>
                <p className="text-[10px] text-slate-500">{data.sensor_snapshot.moisture ?? '--'}% Moisture</p>
                <p className="text-[10px] text-slate-500">
                  Rainfall: {data.input_features?.rainfall ?? '--'} mm
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                  <Activity className="w-3.5 h-3.5 text-emerald-500" /> {t('cropRecommendation.liveNpk')}
                </span>
                ...
                <p className="font-mono font-bold text-slate-800 text-xs">
                  P: <span>{data.sensor_snapshot.phosphorous ?? 0}</span>
                </p>
                <p className="font-mono font-bold text-slate-800 text-xs">
                  K: <span>{data.sensor_snapshot.potassium ?? 0}</span>
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                  <Zap className="w-3 h-3 text-amber-500" /> {t('cropRecommendation.soilEc')}
                </span>
                <p className="font-black text-slate-800 text-sm">{data.sensor_snapshot.ec ?? '--'} µS/cm</p>
                <p className="text-[10px] text-slate-500">
                  {data.weather.rain_mm > 0 ? `${data.weather.rain_mm} mm ${t('cropRecommendation.liveRain')}` : t('cropRecommendation.dryConditions')}
                </p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {data.recommendations.map((item, idx) => {
                const rankKeys = [
                  'cropRecommendation.rank1',
                  'cropRecommendation.rank2',
                  'cropRecommendation.rank3',
                ];
                const badgeColors = [
                ...
                return (
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black shadow-xs ${badgeColors[idx] || badgeColors[2]}`}>
                          {t(rankKeys[idx] || 'cropRecommendation.recommended')}
                        </span>
                        {confidenceVal > 0 && (
                          <span className="text-xs font-black font-mono text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                            {confidenceVal}% {t('cropRecommendation.match')}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Agronomic justification / reason from AI model */}
                      {item.reason && (
                      ...
                      {confidenceVal > 0 && (
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px] font-bold text-slate-400">
                            <span>{t('cropRecommendation.rfProbability')}</span>
                      ...
                      {item.ideal_profile && (
                        <div className="pt-2 border-t border-slate-100 space-y-1.5">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                            {t('cropRecommendation.benchmarks')}
                          </span>
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-semibold">
                      <span>{t('cropRecommendation.rfClassifier')}</span>
                      <span className="text-emerald-700 font-bold">{t('cropRecommendation.liveTelemetryBadge')}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {data.recommendations.map((rec_item, rec_idx) => {
                ...
                const fert_rec = rec_item.fertilizer_recommendation;

                // Fallback calculations if backend fert_rec was not supplied
                ...
                const ideal_n = Number(rec_item.ideal_profile?.ideal_N ?? 0);
                const ideal_p = Number(rec_item.ideal_profile?.ideal_P ?? 0);
                const ideal_k = Number(rec_item.ideal_profile?.ideal_K ?? 0);
                const n_def = Math.max(0, ideal_n - curr_n);
                const p_def = Math.max(0, ideal_p - curr_p);
                const k_def = Math.max(0, ideal_k - curr_k);
                const fallback_urea_qty = (n_def / 0.46).toFixed(1);
                const fallback_dap_qty = (p_def / 0.46).toFixed(1);
                const fallback_mop_qty = (k_def / 0.60).toFixed(1);
                const fallback_no_fert = n_def <= 0 && p_def <= 0 && k_def <= 0;

                const hasStructuredFert = Boolean(fert_rec);
                const isNoFertNeeded = hasStructuredFert ? fert_rec?.no_fertilizer_needed : fallback_no_fert;
                ...
                return (
                  ...
                  >
                    <div className="space-y-4">
                      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                        <div>
                          <span className="text-[10px] font-black uppercase tracking-wider text-emerald-800">
                            {t('cropRecommendation.cropLabel')} {crop_name}
                          </span>
                        ...
                        {conf_val > 0 && (
                          <span className="text-xs font-black font-mono text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                            {conf_val}% {t('cropRecommendation.confidenceLabel')}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <span className="text-[11px] font-black uppercase tracking-wider text-slate-900 block">
                          {t('cropRecommendation.fertilizerToApply')}
                        </span>
                        ...
                        {isNoFertNeeded && (
                          <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200/80 text-xs font-bold text-emerald-900 flex items-start gap-2">
                            ...
                            <span>{fert_rec?.no_fertilizer_message || t('cropRecommendation.soilNutrientsIdeal')}</span>
                          </div>
                        )}

                        <div className="space-y-1.5">
                          {/* Structured Items from Backend */}
                          {hasStructuredFert && fert_rec?.items && fert_rec.items.map((item, it_idx) => {
                            const isUrea = item.type === 'urea';
                            const isDap = item.type === 'dap';
                            const bgStyle = isUrea 
                              ? 'bg-blue-50/80 border-blue-200/70'
                              : isDap 
                              ? 'bg-indigo-50/80 border-indigo-200/70'
                              : 'bg-teal-50/80 border-teal-200/70';
                            const pillStyle = isUrea
                            ...
                            return (
                              <div
                                key={it_idx}
                                className={`p-2.5 rounded-xl border text-xs text-slate-800 flex items-start justify-between gap-2 ${bgStyle}`}
                              >
                                <div>
                                ...
                                <span className={`text-[10px] font-mono font-bold bg-white px-2 py-0.5 rounded border shrink-0 ${pillStyle}`}>
                                  {item.grade}
                                </span>
                              </div>
                            );
                          ...
                          {/* Structured pH Amendment from Backend */}
                          {hasStructuredFert && fert_rec?.ph_amendment && (
                            <div className="p-2.5 bg-amber-50/80 rounded-xl border border-amber-200/70 text-xs text-amber-950 space-y-0.5">
                              <span className="font-black block">{fert_rec.ph_amendment.title}</span>
                              <p className="text-[10px] text-amber-800 font-medium leading-tight">
                                {fert_rec.ph_amendment.advice}
                              </p>
                            </div>
                          )}

                          {/* Fallback rendering if backend structured rec not provided */}
                          {!hasStructuredFert && (
                            <>
                              {n_def > 0 && (
                                <div className="p-2.5 bg-blue-50/80 rounded-xl border border-blue-200/70 text-xs text-slate-800 flex items-start justify-between gap-2">
                                  <div>
                                    <span className="font-black text-blue-950 block">Urea: {fallback_urea_qty} kg/ha</span>
                                  </div>
                              ...
                              {p_def > 0 && (
                                <div className="p-2.5 bg-indigo-50/80 rounded-xl border border-indigo-200/70 text-xs text-slate-800 flex items-start justify-between gap-2">
                                  <div>
                                    <span className="font-black text-indigo-950 block">DAP: {fallback_dap_qty} kg/ha</span>
                                  </div>
                              ...
                              {k_def > 0 && (
                                <div className="p-2.5 bg-teal-50/80 rounded-xl border border-teal-200/70 text-xs text-slate-800 flex items-start justify-between gap-2">
                                  <div>
                                    <span className="font-black text-teal-950 block">MOP: {fallback_mop_qty} kg/ha</span>
                                  </div>
                              ...
                              {curr_ph < 6.0 && (
                                <div className="p-2.5 bg-amber-50/80 rounded-xl border border-amber-200/70 text-xs text-amber-950 space-y-0.5">
                                  <span className="font-black block">Agricultural Lime</span>
                                </div>
                              )}
                              {curr_ph > 7.5 && (
                                <div className="p-2.5 bg-amber-50/80 rounded-xl border border-amber-200/70 text-xs text-amber-950 space-y-0.5">
                                  <span className="font-black block">Gypsum or Sulfur</span>
                                </div>
                              )}
                            </>
                          )}
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-semibold">
                      <span>{fert_rec?.standards?.icar_standard || t('cropRecommendation.icarStandard')}</span>
                      <span className="text-emerald-700 font-bold">
                        {fert_rec?.standards?.live_soil_telemetry || t('cropRecommendation.liveSoilTelemetry')}
                      </span>
                    </div>
                  </div>
                );
          ...
          {onNavigateToDiagnosis && (
            <div className="bg-gradient-to-r from-agri-900 to-agri-950 text-white rounded-3xl p-6 shadow-xl border border-agri-700/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h4 className="text-base font-black text-citrus-300">
                  {t('cropRecommendation.readyToMonitor')}
                </h4>
                <p className="text-xs text-agri-100 mt-0.5">
                  {t('cropRecommendation.readyToMonitorDesc')}
                </p>
              </div>
              <button
                type="button"
              ...
              >
                <span>{t('cropRecommendation.goToDiagnosis')}</span>
